...
var update_client = function(new_version, cb) {
  ...
  if (process.platform === 'win32') {
    bin_path = join(system.paths.package, 'bin', 'node.exe');
    args = [join('lib', 'conf', 'cli.js'), 'config', 'upgrade', new_version];
  } else {
    bin_path = system.paths.package_bin; // /foo/bar/bin/prey
    args = ['config', 'upgrade', new_version];
  }
...
exports.check_every = function(interval, cb) {
  ...
  interval = interval || 60 * 60 * 1000; // one hour by default
