...
function hooks_manager(method) {

  var allowed_method = ["on", "remove"];
  ...
  var hooks_list = {
    'action': sender.notify_action,
    'event': sender.notify_event,
    'data': sender.send_data,
    'report': sender.send_report
  };

  var keys = Object.keys(hooks_list),
      keys_len = keys.length, i, current_key;

  for (i = 0; i < keys_len; i++) {
    current_key = keys[i];
    hooks[method](current_key, hooks_list[current_key]);
  }
...
exports.load = function(cb) {
  ...
  var init_opts = common.config.all();
